%Prints the input maps formed in the PI layer before and after training
%Any one of the Proprioceptive input elements is set to a non-zero value 
%at a time, and the corresponding elements in PI with activation above a
...
Mweights = 0.3*(ones(400,400)); %the intra-layer weights matrix
...
Pweights = 0.2 * (ones(400,400));        %stores intra-layer weights; 400X400
...
%input_to_arm(1,1) = 0.8; %set the activation of Extensor length to 0.3
...
indexP = find(PIlinear>0.55);
PIlinear_print_before = zeros(400,1);
PIlinear_print_before(indexP)=1; %set those elements to 1 whose activation values are above threshold
PIlinear_print_before = (reshape(PIlinear_print_before, 20, 20))'; %reshapes 400X1 to 20X20

%Print MI input map before training
indexM = find(MIlinear>0.55);
MIlinear_print_before = zeros(400,1);
MIlinear_print_before(indexM)=1; %set those elements to 1 whose activation values are above threshold
...
for z=1:100 %learning cycles
    ...
    fprintf('%s','a') %prints 'a' after every cycle of learning is completed
    
end %2000 input pattern cycles

%Print the PI input map after training is completed
indexPa = find(PIlinear>0.55);
PIlinear_print_after = zeros(400,1);
PIlinear_print_after(indexPa)=1 ;%set those elements to 1 whose activation values are above threshold


%Print MI input map after training
indexMa = find(MIlinear>0.55);
MIlinear_print_after = zeros(400,1);
MIlinear_print_after(indexMa)=1; %set those elements to 1 whose activation values are above threshold

PIlinear_print_after = (reshape(PIlinear_print_after, 20, 20))' %reshapes 400X1 to 20X20
